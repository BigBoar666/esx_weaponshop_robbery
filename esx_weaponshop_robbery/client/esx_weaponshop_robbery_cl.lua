...
local vetrine = {
	{x = 20.3, y = -1106.19, z = 29.8, heading = 343.02, isOpen = false},--
	{x = 22.08, y = -1106.76, z = 29.8, heading = 335.8, isOpen = false},--
	{x = 22.38, y = -1107.59, z = 29.8, heading = 254.13, isOpen = false},--
	{x = 22.67, y = -1109.74, z = 29.8, heading = 287.63, isOpen = false},--
	{x = 18.43, y = -1110.15, z = 29.8, heading = 66.87, isOpen = false},--
	{x = 18.01, y = -1111.91, z = 29.8, heading = 63.29, isOpen = false},--
}
...
RegisterNetEvent('esx_ase_robbery:currentlyrobbing')
AddEventHandler('esx_ase_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('esx_ase_robbery:killblip')
AddEventHandler('esx_ase_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_ase_robbery:setblip')
AddEventHandler('esx_ase_robbery:setblip', function(position)
...
RegisterNetEvent('esx_ase_robbery:toofarlocal')
AddEventHandler('esx_ase_robbery:toofarlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('esx_ase_robbery:robberycomplete')
AddEventHandler('esx_ase_robbery:robberycomplete', function(robb)
	holdingup = false
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(Stores)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not holdingup then
					DrawMarker(1, v.position.x, v.position.y, v.position.z-0.9, 0, 0, 0, 0, 0, 0, 1.001, 1.0001, 0.5001, 255, 0, 0, 200, 0, 0, 0, 0)
						...
						if IsPedShooting(GetPlayerPed(-1)) then
							if Config.NeedBag then
							    if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
							        ESX.TriggerServerCallback('esx_ase_robbery:conteggio', function(CopsConnected)
								        if CopsConnected >= Config.RequiredCopsRob then
							                TriggerServerEvent('esx_ase_robbery:rob', k)
		...
		if holdingup then
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
					    ...
					    TriggerServerEvent('esx_ase_robbery:gioielli')
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerServerEvent('esx_ase_robbery:endrob', store)
						    holdingup = false
						    StopSound(soundid)
						end
					end
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 24.14, -1105.76, 29.8, true) > 11.5 ) then
				TriggerServerEvent('esx_ase_robbery:toofar', store)
				holdingup = false
				for i,v in pairs(vetrine) do 
					v.isOpen = false
					vetrineRotte = 0
